...
To the best of our knowledge, our OSR framework is the first that supports OSR point insertion at arbitrary locations in the code. In order to support OSR at any program points, two engineering aspects are involved in the continuation function generation: manipulating the code of the optimized function, which can be daunting when the SSA form must be preserved, and providing a front-end with a clean interface to specify glue code possibly required to perform the transition. There was considerable implementation and experimentation effort to show its feasibility.

Choosing where it is profitable to insert OSR points, also exploiting \osrkit's ability to insert them at places that other solutions do not support, is delegated to the front-end as it stricly depends on the optimizations it performs. In principle, supporting OSR at arbitrary locations can be useful to implement a flexible deoptimization mechanism for aggressive speculative optimizations: e.g., execution might leave a heavily optimized loop while in the middle of it, execute a glue code to adjust the state when needed, and continue in a safe version of the function. 
